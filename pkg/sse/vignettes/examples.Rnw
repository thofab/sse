...
Two methods called \textsf{n} and \textsf{theta} allow to extract the elements of the object individually during evaluation.
...
The function \textsf{powCalc} evaluates the function we created in the last step using the \textsf{powPar} object of the first step.
